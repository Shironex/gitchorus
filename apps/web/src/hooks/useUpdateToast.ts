...
import { GITHUB_RELEASES_URL, UPDATE_ERROR_RELEASE_PENDING } from '@gitchorus/shared';
import { useUpdateStore } from '@/stores/useUpdateStore';
import { useSettingsStore } from '@/stores/useSettingsStore';
import { IS_MAC } from '@/lib/platform';
import type { UpdateStatus } from '@gitchorus/shared';
  ...
  useEffect(() => {
    ...
    if (status === 'available' && updateInfo) {
      ...
      if (IS_MAC) {
        ...
        toast.info(title, {
          description: 'A new version of GitChorus is ready to download.',
          action: {
            label: 'View',
