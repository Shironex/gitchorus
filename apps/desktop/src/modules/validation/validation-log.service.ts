...
const logger = createLogger('ValidationLogService');
...
/**
 * NestJS Injectable that provides file-based log transport for validation.
 *
 * Writes JSONL logs to {app.getPath('userData')}/logs/validation-{date}.log.
 * Rotates daily and retains the last 7 days of logs.
 */
@Injectable()
export class ValidationLogService implements OnModuleDestroy {
  ...
  private getLogFilePath(date: string): string {
    return path.join(this.logDir, `validation-${date}.log`);
  }
  ...
  private cleanupOldLogs(): void {
    try {
      ...
      const logPattern = /^validation-(\d{4}-\d{2}-\d{2})\.log$/;
